...
@njit(parallel=True)
def compute_wq(t, sol, par, compute_w=False, compute_q=False, compute_l=False):
    """ compute the post-decision functions w, q, and/or l """

    # unpack (helps numba optimize)
    w = sol.w
    q = sol.q
    l = sol.l  # unpack labor choice
    ...
    for ip in prange(par.Np):  # in parallel
        ...
        if compute_l:  # allocate container and initialize at zero for labor choice
            l[ip, :] = 0
        ...
        for ishock in range(par.Nshocks):
            ...
            # l_plus
            if compute_l:
                linear_interp.interp_2d_only_last_vec_mon(prep, par.grid_p, par.grid_m, sol.l[t + 1], p_plus, m_plus, l_plus)
            ...
            if compute_q:
                for ia in range(par.Na):
                    q[ip, ia] += weight * par.R * par.beta * utility.marg_func_c(c_plus[ia], par)
            if compute_l:
                for ia in range(par.Na):
                    l[ip, ia] += weight * l_plus[ia]  # accumulate labor choice
        ...
        for ia in range(par.Na):
            ...
            for il in range(par.Nl):
                ...
                for ishock in range(par.Nshocks):
                    ...
                    if compute_w:
                        w[ip, ia] += weight * par.beta * linear_interp.interp_3d(par.grid_p, par.grid_m, par.grid_l, sol.v[t+1], p_plus, m_plus, _l)
                    if compute_q:
                        c_plus_temp = linear_interp.interp_3d(par.grid_p, par.grid_m, par.grid_l, sol.c[t+1], p_plus, m_plus, _l)
                        q[ip, ia] += weight * par.R * par.beta * utility.marg_func(c_plus_temp, _l, par)
