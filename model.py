...
class model_bufferstock():
    ...
    def setup(self):
        ...
        par.rho = 2
        par.beta = 0.96
        par.chi = 0.9  # parameter for disutility of labor
        par.eta = 1.5  # parameter for disutility of labor
    ...
    def solve(self):
        ...
        sol.l = np.nan+np.zeros(shape)  # Initialize labor

        # Last period, (= consume all) 
        sol.m[par.T-1,:]=np.linspace(0,par.a_max,par.Na+1)
        sol.c[par.T-1,:]= sol.m[par.T-1,:].copy()
        sol.l[par.T-1,:] = par.L[-1] * np.ones(par.Na+1)  # Assuming fixed labor supply in the last period
        ...
        for t in range(par.T-2,-1,-1):
            ...
            # add zero consumption and labor
            sol.m[t,0] = par.a_min[t,0]
            sol.c[t,0] = 0
            sol.l[t,0] = 0  # or some other value if not zero
    ...
    def simulate(self):
        ...
        sim.l = np.nan + np.zeros(shape)  # Initialize labor

        # Shocks
        shocki = np.random.choice(par.Nshocks, (par.T, par.simN), replace=True, p=par.w)
        ...
        for t in range(par.simT):
            if par.simlifecycle == 0:
                ...
                sim.l[t, :] = tools.interp_linear_1d(sol.m[0, :], sol.l[0, :], sim.m[t, :])  # Add labor interpolation
            else:
                sim.c[t, :] = tools.interp_linear_1d(sol.m[t, :], sol.c[t, :], sim.m[t, :])
                sim.l[t, :] = tools.interp_linear_1d(sol.m[t, :], sol.l[t, :], sim.m[t, :])  # Add labor interpolation
        ...
        sim.C = sim.c * sim.P
        sim.A = sim.a * sim.P
        sim.L = sim.l * sim.P  # Update the labor supply