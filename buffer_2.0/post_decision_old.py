...
def compute_wq(t, sol, par, compute_w=False, compute_q=False):
    ...
    for ip in prange(par.Np):  # in parallel
        ...
        for ishock in range(par.Nshocks):
            ...
            # v. next-period cash-on-hand and interpolate
            for ia in range(par.Na):
                m_plus[ia] = par.R * par.grid_a[ia] + y_plus - par.w * p * sol.l[ip,ia,ishock]

            # vi. interpolate v_plus and c_plus
            if compute_w:
                linear_interp.interp_2d_only_last_vec_mon(prep, par.grid_p, par.grid_m, sol.v[t + 1], p_plus, m_plus,
                                                           v_plus)

            if compute_q:
                linear_interp.interp_2d_only_last_vec_mon(prep, par.grid_p, par.grid_m, sol.c[t + 1], p_plus, m_plus,
                                                           c_plus)
                linear_interp.interp_2d_only_last_vec_mon(prep, par.grid_p, par.grid_m, sol.l[t + 1], p_plus, m_plus,
                                                           l_plus)
